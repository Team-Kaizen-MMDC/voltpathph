    ...
    fetch('http://localhost:3001/api/ev-models')
      ...
      .catch(err => {
        ...
        setModels([
          ...
          { id: '3', make: 'Geely', model: 'Geometry C', batteryCapacityKWh: 70, averageConsumptionKWhPerKm: 0.17, plugTypes: ['Type 2', 'CCS2'] },
        ]);
        setLoading(false);
      });
  }, []);
        ...
        {models.map(model => (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
              <div style={{ backgroundColor: '#DCFCE7', padding: '8px', borderRadius: '10px' }}>
                <Zap size={18} color="#22C55E" />
              </div>
              <h3 style={{ margin: 0, fontSize: '1.1rem', color: '#0F172A', fontWeight: 600 }}>{model.make} {model.model}</h3>
            </div>
