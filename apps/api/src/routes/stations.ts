...
router.get("/nearby", async (req, res) => {
  ...
  try {
    const stations = await AppDataSource.getRepository(ChargingStation)
      ...
      .where(
        "ST_DWithin(station.location, ST_SetSRID(ST_Point(:lng, :lat), 4324), :radius)",
        { lng: parseFloat(lng as string), lat: parseFloat(lat as string), radius: parseInt(radius as string) }
      )
      .getMany();
    res.json(stations);
  } catch (error) {
